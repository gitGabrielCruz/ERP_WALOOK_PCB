...
- ✅ HU-M06-08: Unificar clientes (Fusión de duplicados)
...
| **RNF-04** | Consistencia visual | Diseño uniforme | Patrón Master-Detail consistente con Usuarios |
...
| Campo "RFC" | `<input type="text">` | Filtro exacto por RFC | `change` | `GET /api/clientes?rfc={rfc}` |
...
| Fusión de duplicados | `.btn-warning` | `puede_eliminar` | Abrir modal de fusión | `POST /api/clientes/fusionar` |
...
CREATE TABLE cliente (
    -- Identificación
    id_cliente          VARCHAR(20) PRIMARY KEY,           -- CLT-00001, CLT-00002, etc.
...
| **13** | **Botón "Fusión de duplicados"** | `puede_eliminar` | Habilitado, abre modal de fusión | `disabled=true` + clase `.btn-disabled` + tooltip "Requiere permisos de administrador" | `POST /api/clientes/fusionar` retorna 403 |
...
**Regla 6: Fusión de Duplicados**
```pseudocode
AL FUSIONAR CLIENTES:
    Validar permiso puede_eliminar
    
    SI permiso == FALSE THEN
        Retornar error 403
        BREAK
    ENDIF
    
    Validar cliente_maestro existe
    Validar clientes_duplicados existen
    Validar cliente_maestro NOT IN clientes_duplicados
    
    BEGIN TRANSACTION
        // Actualizar todas las referencias
        UPDATE venta SET id_cliente = cliente_maestro WHERE id_cliente IN (duplicados)
        UPDATE cita SET id_cliente = cliente_maestro WHERE id_cliente IN (duplicados)
        UPDATE expediente SET id_cliente = cliente_maestro WHERE id_cliente IN (duplicados)
        
        // Marcar duplicados como fusionados
        UPDATE cliente SET fusionado_con = cliente_maestro, estatus = 'INACTIVO' 
        WHERE id_cliente IN (duplicados)
        
        // Registrar auditoría
        INSERT INTO auditoria (accion, tabla, registro_id, usuario, detalles)
        VALUES ('FUSION', 'cliente', cliente_maestro, usuario_actual, JSON_duplicados)
    COMMIT TRANSACTION
