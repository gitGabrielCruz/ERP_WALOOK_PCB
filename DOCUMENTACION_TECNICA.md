...
## 6. Módulo de Inventarios y Optimización UI (ETAPA 2)

### 6.1 Diseño de Alta Densidad Ergonómica
Para maximizar la eficiencia operativa en la gestión de productos, se implementó un diseño de **Ergo-Density**:
*   **Capacidad Visual**: La tabla maestra de productos está fijada a una altura de `980px`, permitiendo la visualización de **11 registros simultáneos** sin necesidad de desplazamiento.
*   **Layout Simétrico**: El contenedor principal utiliza un `split-layout` con una distribución de ancho de **50/50** (`1fr 1fr`), garantizando que la ficha técnica y el catálogo tengan el mismo peso visual.

### 6.2 Gestión de Identidad del Producto
*   **Código de Barras**: Integración de campo `codigo_barras` en el flujo de Datos Maestros para soporte de lectores láser UPC/EAN.
*   **Baja Lógica (Inactivación)**: Se eliminó la funcionalidad de borrado directo desde el catálogo para prevenir pérdida accidental de datos. La inactivación se realiza mediante el cambio de estatus en la ficha técnica, asegurando la integridad de los registros históricos en el Kardex.

### 6.3 Adaptabilidad (Responsive Design)
*   Se redefinió el orden de apilamiento en dispositivos móviles para priorizar el flujo de búsqueda: 1. Filtros → 2. Catálogo → 3. Detalle/Kardex.
