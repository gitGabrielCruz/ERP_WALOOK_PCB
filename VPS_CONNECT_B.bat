...
:: ============================================================
:: VPS_CONNECT_B.bat v4.0
:: Proyecto: Sistema Web ERP en la nube - OMCGC
:: Proposito: Conexion SSH al VPS con keep-alive y dashboard
::            en ventana separada.
:: Autor: Ing. Gabriel Amilcar Cruz Canto
:: Version: 4.0-B (Dashboard en ventana separada)
...
set "RECONEXIONES=0"
set "RECON_FILE=%TEMP%\vps_recon_b.txt"
set "DASHBOARD_PS1=%TEMP%\vps_dashboard_b.ps1"
...
echo.
echo  ============================================================
echo     VPS - ERP WALOOK / OMCGC  [v4.0-B]
...
:: --- SUBIR ARCHIVOS ---
echo   [3/5] Subiendo scripts al VPS...
if "%USE_SSH_KEY%"=="1" (
...
:: --- CREAR DASHBOARD POWERSHELL ---
echo   [4/5] Lanzando dashboard...

> "%DASHBOARD_PS1%" (
    echo $host.UI.RawUI.WindowTitle = 'VPS Dashboard - ERP WALOOK'
    echo $startTime = Get-Date
    echo $startStr = $startTime.ToString('dd/MM/yyyy HH:mm:ss'^)
    echo $server = '%VPS_HOST%:%VPS_PORT%'
    echo $reconFile = '%RECON_FILE%'
    echo.
    echo while ($true^) {
    echo     Clear-Host
    echo     $r = 0
    echo     if (Test-Path $reconFile^) { try { $r = [int](Get-Content $reconFile -Raw^).Trim(^) } catch {} }
    echo     $elapsed = (Get-Date^) - $startTime
    echo     $h = [math]::Floor($elapsed.TotalHours^).ToString('00'^)
    echo     $m = $elapsed.Minutes.ToString('00'^)
    echo     $s = $elapsed.Seconds.ToString('00'^)
    echo     $now = Get-Date -Format 'dd/MM/yyyy HH:mm:ss'
    echo     $sshUp = $null -ne (Get-Process -Name 'ssh' -ErrorAction SilentlyContinue^)
    echo     if ($sshUp^) { $est = 'CONECTADO'; $col = 'Green' } else { $est = 'DESCONECTADO'; $col = 'Red' }
    echo.
    echo     Write-Host ''
    echo     Write-Host '  +==============================================+' -ForegroundColor Cyan
    echo     Write-Host '  ^|   VPS Dashboard  -  ERP WALOOK / OMCGC      ^|' -ForegroundColor Cyan
    echo     Write-Host '  ^|   DevLabs Division by Graxsoft               ^|' -ForegroundColor DarkCyan
    echo     Write-Host '  +==============================================+' -ForegroundColor Cyan
    echo     Write-Host ''
    echo     Write-Host -NoNewline '    Servidor .......... ' -ForegroundColor Gray
    echo     Write-Host $server -ForegroundColor White
    echo     Write-Host -NoNewline '    Conexion inicial .. ' -ForegroundColor Gray
    echo     Write-Host $startStr -ForegroundColor Yellow
    echo     Write-Host -NoNewline '    Hora actual ....... ' -ForegroundColor Gray
    echo     Write-Host $now -ForegroundColor Green
    echo     Write-Host ''
    echo     Write-Host -NoNewline '    Tiempo activo ..... ' -ForegroundColor Gray
    echo     Write-Host "${h}h ${m}m ${s}s" -ForegroundColor Green
    echo     Write-Host -NoNewline '    Reconexiones ...... ' -ForegroundColor Gray
    echo     if ($r -eq 0^) { Write-Host $r -ForegroundColor Green } else { Write-Host $r -ForegroundColor Yellow }
    echo     Write-Host -NoNewline '    Estado ............ ' -ForegroundColor Gray
    echo     Write-Host $est -ForegroundColor $col
    echo     Write-Host ''
    echo     Write-Host '  +----------------------------------------------+' -ForegroundColor DarkGray
    echo     Write-Host '    Actualiza cada segundo - Cerrar con Ctrl+C' -ForegroundColor DarkGray
    echo     Write-Host ''
    echo     Start-Sleep -Seconds 1
    echo }
)

:: Lanzar dashboard en ventana separada
start "VPS Dashboard" powershell -NoProfile -ExecutionPolicy Bypass -File "%DASHBOARD_PS1%"
echo   [OK] Dashboard abierto

:: --- CONECTAR ---
echo   [5/5] Conectando al shell...
echo.
echo  ============================================================
