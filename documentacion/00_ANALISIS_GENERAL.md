...
# 📊 ANÁLISIS GENERAL DEL SISTEMA - ERP WALOOK (M00)
...
El diseño se basa en una arquitectura modular y escalable, sustentada en tecnologías actuales como Java (Spring Boot), JavaScript y MySQL, además de incorporar medidas de seguridad, cifrado de datos, respaldos automáticos y conectividad optimizada en la nube.
...
# 📝 PROTOCOLO DE OPERACIÓN Y BITÁCORA LOG
...
3.	**Figura 6. Diagrama de Despliegue — Arquitectura ERP.** Expone la infraestructura de ejecución del sistema en la nube, identificando los nodos de aplicación, el servidor de base de datos MySQL y los servicios externos de facturación CFDI. También muestra los canales seguros de comunicación (HTTPS/TLS) y los elementos de red asociados al despliegue. (véase Anexo B — Diagrama de Despliegue — Arquitectura ERP).
...
cloud "Internet" as INTERNET {
  ...
  node "Pasarela de Pago" as PAY
  node "PAC / SAT CFDI" as SAT
  node "Proveedor de Mensajería\n(Email / WhatsApp / SMS)" as MSG
}
...
package "Inventario" #F4E8FF {
  ...
  class producto {
    ...
    +precio_venta : decimal(10,2) <<Generated>>
...
producto }o--|| cat_grupo
producto }o--|| cat_familia
producto }o--|| cat_marca
cat_familia }o--|| cat_grupo
...
| | sku | varchar(50) | Sí | UK | SKU único (75XXXXX). |
| | nombre | varchar(200) | Sí | — | Nombre comercial. |
| | id_grupo | UUID | Sí | FK | Grupo (-> cat_grupo). |
| | id_familia | UUID | Sí | FK | Familia (-> cat_familia). |
| | id_marca | UUID | Sí | FK | Marca (-> cat_marca). |
| | costo_unitario | decimal(10,2) | Sí | — | Último costo de compra. |
| | precio_venta | decimal(10,2) | Sí | — | Precio calculado (Costo + Utilidad). |
| | clave_prod_serv | varchar(8) | Sí | — | Clave SAT para facturación. |
...
CREATE TABLE cat_familia (
  ...
  id_grupo UUID NOT NULL REFERENCES cat_grupo(id_grupo),
...
CREATE TABLE producto (
  ...
  id_grupo UUID NOT NULL REFERENCES cat_grupo(id_grupo),
  id_familia UUID NOT NULL REFERENCES cat_familia(id_familia),
  id_marca UUID NOT NULL REFERENCES cat_marca(id_marca),
  codigo_barras VARCHAR(50),
