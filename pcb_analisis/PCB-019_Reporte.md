...
| Camino | Ruta Forense |
| :--- | :--- |
| **C1** | I -> N1 -> N2 -> N3(T) -> N4 -> F (IP Nula -> Normalizar) |
| **C2** | I -> N1 -> N2 -> N3(F) -> N5(T) -> N6 -> F (Detalles Nulos -> Normalizar) |
| **C3** | I -> N1 -> N2 -> N7(EXC) -> F (Fallo de Persistencia / Cripto) |
| **C4** | I -> N1 -> N2 -> N3(F) -> N5(F) -> N8 -> F (Éxito Nominal) |

### Paso 4: Matriz de Automatización (Duda Cero)

| Camino | Caso de Prueba | Resultado |
| :--- | :--- | :--- |
| **C1** | IP Origen = null | **SUCCESS** (Set 0.0.0.0) |
| **C2** | Detalles = null | **SUCCESS** (Set "Sin detalles") |
| **C3** | Fallo AES / SQL | Módulo Silencioso (Controlado) |
| **C4** | **Éxito (Auditoría)** | **SUCCESS (Cifrado AES-256)** |
